...
public class Teacher extends Department {
    ...
    public Teacher(int id, java.time.LocalDate createdDate, java.time.LocalDate updatedDate,
        ...
        if (subject.isEmpty()) throw new SchoolDataException("Subject cannot be empty 27193");
        if (!Pattern.matches("^[\\w.-]+@[\\w.-]+\\.\\w+$", teacherEmail))
            throw new SchoolDataException("Invalid email 27193");
        if (!phone.matches("\\d{10}")) throw new SchoolDataException("Phone must be 10 digits 27193");
